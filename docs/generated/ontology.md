...
Schema version **17**. 28 node types, 15 edge types.
...
| **ReportDefinition** | target | §4.1.1 | `mu_ref` <br> string, required <br> `pbir_ref` <br> string <br> `pages` <br> string[] <br> `model_ref` <br> string, required <br> `version` <br> string <br> `validation_state` <br> string |
| **Visual** | target | §4.1.1 | `page` <br> string, required <br> `type` <br> string, required <br> `source_sheet_ref` <br> string <br> `encodings` <br> json <br> `redesign_flag` <br> bool <br> `redesign_reason` <br> string |
...
| **Pattern** | platform | §4.1.1, §4.3 | `name` <br> string, required <br> `class` <br> enum(C1|C2|C3|C4), required <br> `source_signature` <br> json, required <br> `target_template` <br> text, required <br> `provenance` <br> json <br> `promotion_state` <br> enum(CANDIDATE|ACTIVE|RETIRED), required <br> `pass_count` <br> int |
