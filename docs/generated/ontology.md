...
Schema version **29**. 28 node types, 15 edge types.
...
|---|---|---|---|
| `mu_ref` | string | yes |  |
| `class` | enum(FILTER_CONTEXT|NULL_HANDLING|DATE_GRAIN|AGGREGATION|TYPE_COERCION|LOD_SCOPE|TABLE_CALC|SORT_LIMIT|KEY_MISSING|SOURCE_DRIFT|UNKNOWN|VISUAL_REDESIGN) | yes | Spec §11.1. |
| `evidence_ref` | string | no |  |
| `state` | string | no |  |
| `assignee` | string | no |  |
| `decision` | string | no |  |
