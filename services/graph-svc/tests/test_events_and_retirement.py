...
def test_a_notice_is_not_replayed_and_cannot_be_written_on_its_own_by_mistake() -> None:
    """The three halves of the notice/mutation split, stated together. Three notices share
    the outbox today (S1.2.4's own SOURCE_DRIFT, S5.5.2's own PATTERN_RETIRED, S9.1.2's own
    MU_ACCEPTED) -- all named explicitly, so a future notice added without updating this
    set fails loudly rather than silently starting to replay."""
