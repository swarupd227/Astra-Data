"""The Pattern Library — specification §4.3/§9.3, story S5.5.1 (opens F5.5).
...
**What this module is not.** It does not build S5.5.2's own later, explicit scope:
automatic RETIRED demotion on a failure threshold, or the Parity Dashboard flag §9.3
describes for every MU that used a retired pattern — `promotion_status` surfaces a
recorded failure so a human can act on it, but nothing here demotes a pattern on its own.
It does not build a Pattern Library *console screen* — this story's own acceptance
criteria asks for the mechanism, not one, the identical scope boundary S5.2.1/S5.2.2 already
drew for the Pattern Library's own promotion pipeline before this story existed to build it.
"""
...
async def apply_active_pattern(
    ...
    *,
    ...
    a field on one pattern's own hiccup, and the failure counts toward §9.3's own "above a
    threshold ... automatically ... RETIRED" — S5.5.2's own later scope, not built here.
    """
    ...
    if parse_error is not None:
        await record_observation(
            ...
            observed_pass=False, source="DETERMINISTIC_APPLICATION", created_by=principal.value,
        )
        return None
